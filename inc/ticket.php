...
if (validate_user($_SESSION['helpdesk_user_id'], $_SESSION['code'])) {
    ...
    $stmt = $dbConnection->prepare('SELECT 
							id, user_init_id, user_to_id, date_create, subj, msg, client_id, unit_id, status, hash_name, comment, last_edit, is_read, lock_by, ok_by, arch, ok_date, prio, last_update, err, tdate
							from tickets
							where hash_name=:hn');
    ...
    if (!empty($res1)) {
        foreach($res1 as $row) {
			...
			else if ($row['prio'] == "-1") {$prio= "<span class=\"label label-warning\"><i class=\"fa fa-arrow-down\"></i> ".lang('t_list_a_p_low')."</span>"; $prio_style['normal']="active";}
			else if ($row['prio'] == "-2") {$prio= "<span class=\"label label-warning\"><i class=\"fa fa-arrow-down\"></i> ".lang('t_list_a_p_high')."</span>"; $prio_style['high']="active";}			

            //else if ($row['prio'] == "3") {$prio= "<span class=\"label label-warning\"><i class=\"fa fa-minus-circle\"></i> ".lang('t_list_a_p_error')."</span>"; $prio_style['error']="active";}



            ?>
<input type="hidden" id="prio" value="<?=$row['prio'];?>">
<input type="hidden" id="err" value="<?=$row['err'];?>">
<input type="hidden" id="tdate" value="<?=$row['tdate'];?>">
...
<?php } ?>
            ...
            <table class="table table-bordered">
				...
				<? $tekdate = date("Y-m-d");
				$t1date = date("d.m.Y",strtotime($tdate));
                             				
				if ($tdate > $tekdate) {?>					
                <tr>
                    <td><small><strong>Отложено до:</strong></small></td>
                    <td><strong><?=$t1date;?></strong></td>
                    <td></td>
                    <td></td>
                </tr>
				<?php } ?>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-10" style=" padding-top: 5px; ">
				...
				-->	
					...
					<!--
                    <div class="btn-group">
                        <button type="button" class="btn btn-danger btn-xs <?=$prio_style['error'];?>" id="prio_error"><i id="lprio_error" class=""></i><?=lang('NEW_prio_error');?></button>
                    </div>
					-->		
                </div>
            </div></div></div></div>
...
<div class="control-group">
    <div class="controls">
        <div class="form-group" id="for_tdate">
            <label for="tdate" class="col-sm-2 control-label"><small><?=lang('NEW_TDATE');?>:</small></label>
            <div class="col-sm-10">
		<input type="date" class="form-control input-sm" name="tdate" id="tdate_up" value="<?=$tdate;?>" placeholder="<?=lang('NEW_TDATE_msg');?>" data-toggle="popover" data-html="true" data-trigger="manual" data-placement="right"  data-content="<small><?=lang('NEW_TDATE_msg');?></small>">
            </div>
        </div>
                                        ...
                                        <div class="controls">
                                            <div class="form-group" id="for_msg">
                                                ...
                                                <div class="col-sm-12" style="">
                                                    ...
                                                    <textarea data-toggle="popover" data-html="true" data-trigger="manual" data-placement="right" data-content="&lt;small&gt;<?=lang('TICKET_t_det_ticket');?>&lt;/small&gt;" placeholder="<?=lang('TICKET_t_comm_ph');?>" class="form-control input-sm animated" name="msg" id="msg" rows="1" aria-invalid="false"></textarea>
