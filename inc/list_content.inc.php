...
if (isset($_SESSION['hd.rustem_sort_out'])) {
	if ($_SESSION['hd.rustem_sort_out'] == "ok"){
		$stmt = $dbConnection->prepare('SELECT id, user_init_id, user_to_id, date_create, subj, msg, client_id, unit_id, status, hash_name, is_read,lock_by, ok_by, prio, err from tickets where unit_id=7 and arch=:n and status=:s limit :start_pos, :perpage');
        $stmt->execute(array(':s'=>'1', ':n'=>'0',':start_pos'=>$start_pos,':perpage'=>$perpage));
	}
	
	else if ($_SESSION['hd.rustem_sort_out'] == "free"){
		$stmt = $dbConnection->prepare('SELECT id, user_init_id, user_to_id, date_create, subj, msg, client_id, unit_id, status, hash_name, is_read,lock_by, ok_by, prio, err, tdate from tickets where unit_id=7 and arch=:n and lock_by=:lb and status=:s limit :start_pos, :perpage');
        $stmt->execute(array(':lb'=>'0',':s'=>'0', ':n'=>'0',':start_pos'=>$start_pos,':perpage'=>$perpage));
	}
	
	
	else if ($_SESSION['hd.rustem_sort_out'] == "ilock"){
		$stmt = $dbConnection->prepare('SELECT id, user_init_id, user_to_id, date_create, subj, msg, client_id, unit_id, status, hash_name, is_read,lock_by, ok_by, prio, err, tdate from tickets where unit_id=7 and arch=:n and lock_by=:lb limit :start_pos, :perpage');
        $stmt->execute(array(':lb'=>$user_id, ':n'=>'0',':start_pos'=>$start_pos,':perpage'=>$perpage));
	}
	
	else if ($_SESSION['hd.rustem_sort_out'] == "lock"){
		$stmt = $dbConnection->prepare('SELECT id, user_init_id, user_to_id, date_create, subj, msg, client_id, unit_id, status, hash_name, is_read,lock_by, ok_by, prio, err, tdate from tickets where unit_id=7 and arch=:n and (lock_by<>:lb and lock_by<>0) and (status=0) limit :start_pos, :perpage');
        $stmt->execute(array(':lb'=>$user_id, ':n'=>'0',':start_pos'=>$start_pos,':perpage'=>$perpage));
	}
}
...
if (!isset($_SESSION['hd.rustem_sort_out'])) {        
	...
	$stmt = $dbConnection->prepare('SELECT 
                                                       id, user_init_id, user_to_id, date_create, subj, msg, client_id, unit_id, status, hash_name, is_read,lock_by, ok_by, prio, err, tdate
                                                       from tickets 
                                                       where unit_id=7 and arch=:n 
        ...
        if ($aha <> "0") { ?>
            ...
            <table id="myTable" class="tbl table table-bordered table-hover tablesorter " style=" font-size: 14px; ">
                <thead>
                <tr>
...
////////////////////////////Показывает приоритет//////////////////////////////////////////////////////////////
                ...
                if ($row['prio'] == "-1") {$prio= "<span class=\"label label-warning\" data-toggle=\"tooltip\" data-placement=\"bottom\" title=\"".lang('t_list_a_p_low')."\"><i class=\"fa fa-arrow-down\"></i></span>"; }
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                    ...
                    <tr id="tr_<?php echo $row['id']; ?>" class="tbl-item <?=$style?>">
                        ...
                        <td class="title" style=" vertical-align: middle; "><small><?php name_of_client($row['client_id']); ?></small></td>
                        <td style=" vertical-align: middle; "><small><center><time id="c" datetime="<?=$row['date_create']; ?>"></time></center></small></td>
                        <td style=" vertical-align: middle; "><small><?php get_unit_name($row['unit_id']); ?></small></td>
...
foreach ($ee as $key=>$value) { $vv[":val_" . $key]=$value;}
   	...
   	a.is_read, a.lock_by, a.ok_by, a.prio, a.last_update, a.arch, c.fio, c.id
        from tickets as a LEFT JOIN  clients as c ON a.client_id=c.id
    	where (a.user_to_id=:n and a.unit_id IN ('.$in_query.') )) or a.user_init_id=:user_id2) and (a.id=:z or a.subj like :z1 or a.msg like :z2 or c.fio like :z3) group by a.id limit 10');
   	...
   	(a.id=:z or a.subj like :z1 or a.msg like :z2 or c.fio like :z3) group by a.id limit 10');
...
$stmt = $dbConnection->prepare('SELECT
   	...
   	a.is_read, a.lock_by, a.ok_by, a.prio, a.last_update, a.arch, c.fio, c.id
   	from tickets as a LEFT JOIN  clients as c ON a.client_id = c.id
    	where a.id=:z or a.subj like :z1 or a.msg like :z2 or c.fio like :z3 group by a.id limit 10');
        ...
        <?php
           ...
           <table id="myTable" class="table table-bordered table-hover tablesorter " style=" font-size: 14px; ">
            <thead>
            <tr>
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                ...
                <tr id="tr_<?php echo $row['id']; ?>" class="<?=$style?>">
                    ...
                    <td style=" vertical-align: middle; "><small class="<?=$muclass;?>"><center><time id="a" datetime="<?=$t_ago;?>"></time>                
                    </center></small></td>
...
//echo $t2date;
							/* and tdate=<' .$t2date. ' */
            $stmt = $dbConnection->prepare('SELECT 
							id, subj, msg, client_id, unit_id, hash_name, lock_by, ok_by, prio, last_update, err, tdate
							from tickets
							where arch=:n and prio IN (-1,-2) and tdate<="'.$t2date.'"
...
//Главный начальник
        ...
        <?php } if ($aha <> "0") {
			...
			<!--<input type="hidden" value="" id="err">-->
			
				<?php
					foreach($res2 as $r) {
                ?>
			
			<input type="hidden" value="<?php echo $r['hash_name']; ?>" id="ticket_hash">
			<input type="hidden" value="<?php echo $r['prio']; ?>" id="prio">
			<input type="hidden" value="<?php echo $r['tdate']; ?>" id="tdate_up">
			<input type="hidden" value="<?php echo $r['msg']; ?>" id="msg_up">
			<input type="hidden" value="<?php echo $r['subj']; ?>" id="subj">
...
<!--вывод списка тикетов-->
            ...
            <tr>
                <th><center><div id="sort_id" action="<?=$_SESSION['helpdesk_sort_id'];?>">#4<?=$id_icon;?></div></center></th>
...
////////////////////////////Показывает err//////////////////////////////////////////////////////////////
				...
				$tekdate = date("d.m.Y");
				$t1date = date("d.m.Y",strtotime($row['tdate']));
				$vtdate ="";
				$outc="";
				
				//echo $t1date. '-'. $tekdate .'<br />'; 
               // if ($t1date > $tekdate) {
						
				//}	 
				$vtdate=($t1date > $tekdate)? $t1date : "";  //проверка даты заявки
				//if ($row['err'] == "1") {$err= "1"; }				
				if ($row['unit_id'] == "7") {$outc="out"; }		//ставим стиль для категории "Выезд" 		
...
////////////////////////////Показывает приоритет//////////////////////////////////////////////////////////////
                ...
                if ($row['prio'] == "-1") {$prio= "<span class=\"label label-warning\" data-toggle=\"tooltip\" data-placement=\"bottom\" title=\"".lang('t_list_a_p_low')."\"><i class=\"fa fa-arrow-down\"></i></span>"; }
                if ($row['prio'] == "2") {$prio= "<span class=\"label label-danger\" data-toggle=\"tooltip\" data-placement=\"bottom\" title=\"".lang('t_list_a_p_high')."\"><i class=\"fa fa-arrow-up\"></i></span>"; }
               if ($row['prio'] == "-2") {$prio= "<span class=\"label label-warning\" data-toggle=\"tooltip\" data-placement=\"bottom\" title=\"".lang('t_list_a_p_low')."\"><i class=\"fa fa-arrow-down\"></i></span>"; }
...
foreach ($ee as $key=>$value) { $vv[":val_" . $key]=$value;}        
 ...
 //$pp2=array_merge($vv,$vv2);     
        ...
        if ($priv_val == 0) {
            ...
            $stmt = $dbConnection->prepare('SELECT 
							id, user_init_id, user_to_id, date_create, subj, msg, client_id, unit_id, status, hash_name, is_read, lock_by, ok_by, ok_date, err, tdate
							from tickets
