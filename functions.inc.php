                                    ...
                                    <div class="panel-body" style="max-height: 400px; scroll-behavior: initial; overflow-y: scroll;">
                                        ...
                                        <table class="table table-hover">
                                            ...
                                            foreach($re as $row) {
												...
												if ($t_action == 'edit_prio') {$icon_action="fa fa-pencil-square"; $text_action=lang('TICKET_t_a_e_prio');}
...
function get_client_info_ticket($id) {
    global $dbConnection;
    $stmt = $dbConnection->prepare('SELECT fio,tel,tel2,tel3,unit_desc,adr,tel_ext,email,email2,email3,cod,site,login,region,posada FROM clients where id=:id');
        ...
        <table class="table  ">
            ...
            <?php } if ($region) { ?>
                <tr>
                    ...
                    <td><small><?php echo $region; ?></small></td>
                </tr>				
            <?php } if ($tel_user) { ?>
                <tr>
            ...
            <?php } if ($email) { ?>
                <tr>
                    ...
                    <td><a href="mailto:<?php echo $email; ?>" target="_blank"><small><?php echo $email; ?></small></a></td>
                </tr>
            <?php } if ($email2) { ?>
                <tr>
                    <td style=" width: 30px; "><a href="mailto:<?=lang('WORKER_mail2');?>" target="_blank"><small><?=lang('WORKER_mail2');?>:</small></a></td>
                    <td><a href="mailto:<?php echo $email2; ?>" target="_blank"><small><?php echo $email2; ?></small></a></td>
                </tr>
            <?php } if ($email3) { ?>
                <tr>
                    <td style=" width: 30px; "><a href="mailto:<?=lang('WORKER_mail3');?>" target="_blank"><small><?=lang('WORKER_mail3');?>:</small></a></td>
                    <td><a href="mailto:<?php echo $email3; ?>" target="_blank"><small><?php echo $email3; ?></small></a></td>
                </tr>
            <?php } if ($cod) { ?>
                <tr>
                    <td style=" width: 30px; "><small><?=lang('WORKER_cod');?>:</small></td>
                    <td><small><?php echo $cod; ?></small></td>
                </tr>
            <?php } if ($site) { ?>
                <tr>
                    <td style=" width: 30px; "><small><?=lang('WORKER_site');?>:</small></a></td>
                    <td><a href="<?php echo $site; ?>" target="_blank"><small><?php echo $site; ?></small></a></td>
                </tr>				
            <?php } ?>
            <tr>
...
function get_client_info($id) {
    ...
    <div class="panel-body">
        ...
        <table class="table  ">
            ...
            <tr>
                <td style=" width: 30px; "><small><?=lang('WORKER_region');?>:</small></td>
                <td><small><a href="#" id="edit_region" data-type="select" data-source="<?=$CONF['hostname'];?>/inc/json.php?region" data-pk="1" data-title="<?=lang('NEW_to_region');?>"><?php echo $region; ?></a></small></td>
            </tr>
            <tr>
                <td style=" width: 30px; "><small><?=lang('WORKER_tel');?>:</small></td>
            ...
            <tr>
                <td style=" width: 30px; "><small><?=lang('WORKER_tel2');?>:</small></td>
                <td><small><a href="#" id="edit_tel2" data-type="text"><?php echo $tel_user2." ".$tel_ext; ?></a></small></td>
            </tr>
            <tr>
                <td style=" width: 30px; "><small><?=lang('WORKER_tel3');?>:</small></td>
                <td><small><a href="#" id="edit_tel3" data-type="text"><?php echo $tel_user3." ".$tel_ext; ?></a></small></td>
            </tr>			
            <tr class="dogovor">
                <td style=" width: 30px; "><small><?=lang('WORKER_room');?>:</small></td>
            ...
            <tr>
                ...
                <td><small><a href="#" id="edit_mail2" data-type="text"><?=$mail2?></a></small></td>
            </tr>
            <tr>
                <td style=" width: 30px; "><small><?=lang('WORKER_mail3');?>:</small></td>
                <td><small><a href="#" id="edit_mail3" data-type="text"><?=$mail3?></a></small></td>
            </tr>
            <tr>
                <td style=" width: 30px; "><small><?=lang('WORKER_cod');?>:</small></td>
                <td><small><a href="#" id="edit_cod" data-type="text"><?=$cod?></a></small></td>
            </tr>
            <tr>
                <td style=" width: 30px; "><small><?=lang('WORKER_site');?>:</small></td>
                <td><small><a href="#" id="edit_site" data-type="text"><?=$site?></a></small></td>
            </tr>			
            <tr>
                <td style=" width: 30px; "><small class="text-muted"><?=lang('WORKER_total');?>:</small></td>
                <td><small class="text-muted">
                        <?php if ($priv_val <> "1") { ?>
                         <a target="_blank" href="userinfo?user=<?=$id?>"><?php }?><?php echo $tt; ?><?php if ($priv_val <> "1") { ?></a> (<?php echo $tt2; ?>)<?php } ?></small></td>
            </tr>
<?php if ( $tt <> 0) { ?>
            <tr>
                <td style=" width: 30px; "><small class="text-muted"><?=lang('WORKER_last');?>:</small></td>
...
if (isset($_SESSION['hd.rustem_sort_in'])) {
	if ($_SESSION['hd.rustem_sort_in'] == "ok"){$stmt = $dbConnection->prepare("SELECT max(last_update) from tickets where ((user_to_id=:id and arch='0') or (user_to_id='0' and unit_id IN (".$in_query.") and arch='0')) and status=:s");
			...
			$max_id=$max[0];
				...
				else if ($_SESSION['hd.rustem_sort_in'] == "free"){$stmt = $dbConnection->prepare("SELECT max(last_update) from tickets where ((user_to_id=:id and arch='0') or (user_to_id='0' and unit_id IN (".$in_query.") and arch='0')) and lock_by=:lb and status=:s");
...
function get_total_tickets_out_and_success() {
    ...
    $res = $dbConnection->prepare("SELECT count(*) from tickets where (ok_by='0') and arch='0'");
    $res->execute();
    $count = $res->fetch(PDO::FETCH_NUM);
...
function get_total_tickets_out_and_lock() {
    ...
    $res = $dbConnection->prepare("SELECT count(*) from tickets where (lock_by!='0' and ok_by='0') and arch='0'");
    $res->execute(array(':uid' => $uid));
    $count = $res->fetch(PDO::FETCH_NUM);
...
foreach ($ee as $key=>$value) { $vv[":val_" . $key]=$value;}
    ...
    if ($priv_val == "0") {
        ...
        $res = $dbConnection->prepare("SELECT count(*) from tickets where unit_id IN (".$in_query.") and status='0' and prio NOT IN (-1,-2) and lock_by='0'");
        ...
        $res = $dbConnection->prepare("SELECT count(*) from tickets where ((user_to_id=:uid and arch='0') or (user_to_id='0' and unit_id IN (".$in_query.") and arch='0')) and status='0' and prio NOT IN (-1,-2) and lock_by='0'");
        ...
        $res = $dbConnection->prepare("SELECT count(*) from tickets where status='0' and prio NOT IN (-1,-2) and lock_by='0'");
        $res->execute();
        $count = $res->fetch(PDO::FETCH_NUM);
        $count=$count[0];
...
function like_match($pattern, $subject)
{
    ...
    //$stmt = $dbConnection->prepare('SELECT c.id, c.adr, t.client_id FROM clients as c, tickets as t where t.client_id=c.id and t.id=:id');
    //$stmt->execute(array(':id' => $id));
